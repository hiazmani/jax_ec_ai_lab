"""Definition of classes needed to define observation, reward and action spaces.

See JAXMARL: https://github.com/FLAIROx/JaxMARL/blob/main/jaxmarl/environments/spaces.py
"""

from collections import OrderedDict
from typing import Sequence, Tuple, Union  # type: ignore
...
class Discrete(Space):
    ...
    def __init__(self, num_categories: int, dtype=jnp.int32) -> None:
        """Initializes jittable discrete space."""
...
class MultiDiscrete(Space):
    ...
    def __init__(self, num_categories: Sequence[int]) -> None:
        """Num categories is the number of cat actions for each dim.

        [2,2,2]=2 actions x 3 dim.
        """
...
class Dict(Space):
    ...
    def flattened_shape(self) -> int:
        """Return the flattened shape of the dictionary."""
        # This assumes that all subspaces are 1D, if any of them are not throw an error
        if any(len(space.shape) != 1 for space in self.spaces.values()):
            raise ValueError("Flattening only supported for 1D subspaces.")
        return sum([space.shape[0] for space in self.spaces.values()])
